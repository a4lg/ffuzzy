...
use crate::internals::hash::{fuzzy_norm_type, FuzzyHashData};
use crate::internals::hash_dual::{
...
impl FuzzyHashCompareTarget {
    ...
    /// [`BlockHashPositionArrayImplMut`](crate::internals::compare::position_array::BlockHashPositionArrayImplMut)
        ...
        BlockHashSizes<S1, S2>: ConstrainedBlockHashSizes,
    {
        ...
        self.block_hash_1_mut()
            .init_from_partial(hash.block_hash_1());
        self.block_hash_2_mut()
            .init_from_partial(hash.block_hash_2());
    }
        ...
        BlockHashSizes<S1, S2>: ConstrainedBlockHashSizes,
    {
        let hash = hash.as_ref();
        self.block_hash_1_internal()
            .is_equiv_internal(hash.block_hash_1())
            && self
                .block_hash_2_internal()
                .is_equiv_internal(hash.block_hash_2())
    }
    ...
    fn raw_score_by_edit_distance_internal(
        ...
        */
        let subscore = (edit_distance * block_hash::FULL_SIZE as u32)
            / (len_block_hash_lhs as u32 + len_block_hash_rhs as u32); // grcov-excl-br-line:DIVZERO
        100 - (100 * subscore) / block_hash::FULL_SIZE as u32
    }
...
impl FuzzyHashCompareTarget {
        ...
        BlockHashSizes<S1, S2>: ConstrainedBlockHashSizes,
    {
        ...
        let (bs1, _bs2) = (self.log_blocksize, other.log_blocksize);
        debug_assert!(!self.is_equiv(other));
        debug_assert!(block_size::is_near_eq(bs1, _bs2));
        u32::max(
            self.block_hash_1_internal()
                .score_strings_internal(other.block_hash_1(), bs1),
            self.block_hash_2_internal()
                .score_strings_internal(other.block_hash_2(), bs1 + 1),
        )
    }
    ...
    {
        let other = other.as_ref();
        let (_bs1, _bs2) = (self.log_blocksize, other.log_blocksize);
        assert!(!self.is_equiv(other));
        assert!(block_size::is_near_eq(_bs1, _bs2));
        self.compare_unequal_near_eq_internal(other)
    }
    ...
    {
        let other = other.as_ref();
        let (_bs1, bs2) = (self.log_blocksize, other.log_blocksize);
        assert!(block_size::is_near_lt(_bs1, bs2));
        self.block_hash_2_internal()
            .score_strings_internal(other.block_hash_1(), bs2)
    }
        ...
        BlockHashSizes<S1, S2>: ConstrainedBlockHashSizes,
    {
        let other = other.as_ref();
        let (_bs1, _bs2) = (self.log_blocksize, other.log_blocksize);
        assert!(block_size::is_near_lt(_bs1, _bs2));
        self.compare_unequal_near_lt_internal(other)
    }
        ...
        BlockHashSizes<S1, S2>: ConstrainedBlockHashSizes,
    {
        let other = other.as_ref();
        let (bs1, _bs2) = (self.log_blocksize, other.log_blocksize);
        assert!(block_size::is_near_gt(bs1, _bs2));
        self.block_hash_1_internal()
            .score_strings_internal(other.block_hash_2(), bs1)
    }
        ...
        BlockHashSizes<S1, S2>: ConstrainedBlockHashSizes,
    {
        let other = other.as_ref();
        let (_bs1, _bs2) = (self.log_blocksize, other.log_blocksize);
        assert!(block_size::is_near_gt(_bs1, _bs2));
        self.compare_unequal_near_gt_internal(other)
    }
        ...
        BlockHashSizes<S1, S2>: ConstrainedBlockHashSizes,
    {
        let other = other.as_ref();
        let (_bs1, _bs2) = (self.log_blocksize, other.log_blocksize);
        debug_assert!(block_size::is_near_eq(_bs1, _bs2));
        self.block_hash_1_internal()
            .has_common_substring_internal(other.block_hash_1())
            || self
                .block_hash_2_internal()
                .has_common_substring_internal(other.block_hash_2())
    }
        ...
        BlockHashSizes<S1, S2>: ConstrainedBlockHashSizes,
    {
        let other = other.as_ref();
        let (_bs1, _bs2) = (self.log_blocksize, other.log_blocksize);
        debug_assert!(block_size::is_near_lt(_bs1, _bs2));
        self.block_hash_2_internal()
            .has_common_substring_internal(other.block_hash_1())
    }
        ...
        BlockHashSizes<S1, S2>: ConstrainedBlockHashSizes,
    {
        let other = other.as_ref();
        let (_bs1, _bs2) = (self.log_blocksize, other.log_blocksize);
        debug_assert!(block_size::is_near_gt(_bs1, _bs2));
        self.block_hash_1_internal()
            .has_common_substring_internal(other.block_hash_2())
    }
...
impl<const S1: usize, const S2: usize> core::convert::From<fuzzy_norm_type!(S1, S2)>
    for FuzzyHashCompareTarget
where
    BlockHashSize<S1>: ConstrainedBlockHashSize,
    BlockHashSize<S2>: ConstrainedBlockHashSize,
    BlockHashSizes<S1, S2>: ConstrainedBlockHashSizes,
...
mod const_asserts {
    ...
    // Regular tests.
    const_assert!(block_size::is_log_valid(
        FuzzyHashCompareTarget::LOG_BLOCK_SIZE_CAPPING_BORDER
    ));
    const_assert!(!is_log_block_size_needs_no_capping(
        FuzzyHashCompareTarget::LOG_BLOCK_SIZE_CAPPING_BORDER - 1
    ));
    const_assert!(is_log_block_size_needs_no_capping(
        FuzzyHashCompareTarget::LOG_BLOCK_SIZE_CAPPING_BORDER
    ));
    ...
    fn log_block_size_capping_border_is_correct() {
        assert!(!is_log_block_size_needs_no_capping(
            FuzzyHashCompareTarget::LOG_BLOCK_SIZE_CAPPING_BORDER - 1
        ));
        assert!(is_log_block_size_needs_no_capping(
            FuzzyHashCompareTarget::LOG_BLOCK_SIZE_CAPPING_BORDER
